...
class TeekoPlayer:
    ...
    def make_move(self, state):
        ...
        if not drop_phase:
            ...
            available_pieces = [(i, j) for i in range(5) for j in range(5) if state[i][j] == self.my_piece]
            piece_to_move = random.choice(available_pieces)

            unoccupied_spaces = [(i, j) for i in range(5) for j in range(5) if state[i][j] == ' ']
            destination_space = random.choice(unoccupied_spaces)

            move = [destination_space, piece_to_move]
        else:
    ...
    def game_value(self, state):
        ...
        for i in range(4):
            for j in range(4):
                ...
                if ' ' not in box and len(set(box)) == 1:
